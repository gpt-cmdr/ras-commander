...
- `captured_real`: unchanged source-project inputs and captured artifacts
  preserved through staging and execution;
...
Successful execution receipts include complete preparation and finalization
cleanup records. In each record, `result_format` names the family targeted for
deletion, not the family selected or generated by the run. `removed_paths` and
`missing_paths` are disjoint and together partition the exact plan-scoped
target set implied by `result_format` and `include_message_sidecars`.
...
- **R04 Exact deletion:** only the plan's allowlisted result and sidecar paths
  are removed during explicit initial-state setup, pre-run preparation, and
  post-run finalization. Structured cleanup records cover transient artifacts
  that are created and removed between filesystem snapshots.
...
For `status`, the backward-compatible top-level `hec_ras_invoked` and the
explicit `status_action_hec_ras_invoked` describe only the read-only status
action. Historical claims appear per attempt and in the tri-state
`any_verified_attempt_hec_ras_invoked`. A status state of `verified_terminal`
means the generic receipt/digest envelope verified; the stricter semantic and
reusability gates still run before resume. Non-boolean historical invocation
claims normalize to unknown rather than becoming truthy values.
