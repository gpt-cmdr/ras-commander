...
| 21 | `15_stored_map_generation.ipynb` | ❌ FAILED | 7 sec | **BLOCKER**: Missing `init_ras_project()` call before GUI automation. Cell 4 raises RuntimeError: "Project not initialized". Notebook attempts `RasGuiAutomation.open_rasmapper()` without extracting/initializing project first. Simple fix: add RasExamples extraction + init_ras_project() call between cells 3-4 (follow pattern from notebook 26). [Details](.claude/outputs/notebook-runner/2025-12-16-15_stored_map_generation-test.md) |
