...
| 20 | `15_c_floodplain_mapping_python_gis.ipynb` | ✅ PASS | 429 sec (7m 9s) | **TESTED 2025-12-16**: All 7 code cells executed successfully in pip environment (rascmdr_piptest). Complete Python-GIS mapping workflow: project extraction, raster generation (WSE/Depth/Velocity), visualization, and time series analysis. Zero errors, clean execution. GIS stack (geopandas, rasterio, shapely) fully integrated with ras-commander HDF results. [Details](.claude/outputs/notebook-runner/2025-12-16-15c_floodplain_mapping_python_gis-test.md) |
| 21 | `15_stored_map_generation.ipynb` | 🔧 FIXED (GUI required) | - | **FIXED 2025-12-17**: Added missing project initialization. New cell after toggle extracts BaldEagleCrkMulti2D (suffix="15"), initializes project, and computes plan 06. Initialization verified - RuntimeError resolved. Timeout during plan computation is expected (5-30+ min for 2D unsteady). GUI automation (RasGuiAutomation.open_rasmapper) requires manual interaction. Fix verified, notebook ready for manual testing. [Details](.claude/outputs/notebook-runner/2025-12-17-15_stored_map_test/) |
