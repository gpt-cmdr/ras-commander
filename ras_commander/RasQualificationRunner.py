...
class RasQualificationRunner:
    ...
    @staticmethod
    def _prepare_wine_long_path_drive(
        prefix: Path,
        project_root: Path,
    ) -> Dict[str, Any]:
        """Provision a prefix-local Wine drive for long project paths."""
        prefix = RasUtils.safe_resolve(Path(prefix))
        project_root = RasUtils.safe_resolve(Path(project_root))
        if not project_root.is_relative_to(prefix):
            raise ValueError("Wine long-path project root must be inside the prefix")
        ...
        for letter in "qponmlkjihgfed":
            ...
            if os.path.lexists(candidate):
                continue
            relative_target = os.path.relpath(project_root, dosdevices)
            candidate.symlink_to(relative_target, target_is_directory=True)
            if RasUtils.safe_resolve(candidate) != project_root:
                candidate.unlink(missing_ok=True)
                raise RuntimeError("Wine long-path drive did not resolve to project root")
            selected = candidate
            break
        if selected is None:
            raise RuntimeError("No free Wine drive letter for long-path project root")
        return {
            "drive": selected.name.upper(),
            "dosdevice": str(selected),
            "project_root": str(project_root),
            "relative_target": os.readlink(selected),
            "resolved_target": str(RasUtils.safe_resolve(selected)),
            "target_within_prefix": project_root.is_relative_to(prefix),
        }
    ...
    def _worker_environment(self, *, native_worker: bool = False) -> Dict[str, str]:
        ...
        if self.context.get("wine_prefix"):
            ...
            long_path_drive = self.context.get("wine_long_path_drive") or {}
            if long_path_drive:
                environment["RAS_COMMANDER_LONG_PATH_ROOT_DRIVE"] = str(
                    long_path_drive["drive"]
                )
                environment["RAS_COMMANDER_LONG_PATH_ROOT"] = (
                    r"C:\ras-qualification-projects"
                )
        environment.update(
            {
    ...
    def _prepare(self) -> None:
        ...
        if self.config.profile == ExecutorProfile.LINUX_WINE_WINDOWS_RAS:
            ...
            if bool(self.config.wine.get("stage_inside_prefix", True)):
                ...
                wine_long_path_drive = self._prepare_wine_long_path_drive(
                    Path(wine_prefix_receipt["prefix"]),
                    stage_root,
                )
                wine_prefix_receipt["long_path_drive"] = wine_long_path_drive
            ...
            self.context.update(
                {
                    ...
                    "wine_long_path_drive": wine_long_path_drive,
                }
            )
