...
internal static class RasMapperMeshHelper
{
    ...
    private static int ResolveFeatureId(object layer, string requestedName)
    {
        MethodInfo byName = layer.GetType().GetMethods().Single(method =>
            method.Name == "GetFeatureByName" &&
            method.GetParameters().Length == 1 &&
            method.GetParameters()[0].ParameterType == typeof(string)
        );
        int fid = Convert.ToInt32(
            byName.Invoke(layer, new object[] { requestedName })
        );
        if (fid >= 0 || requestedName.Length != 16)
            return fid;

        MethodInfo count = layer.GetType().GetMethods().Single(method =>
            method.Name == "FeatureCount" && method.GetParameters().Length == 0
        );
        MethodInfo featureName = layer.GetType().GetMethods().Single(method =>
            method.Name == "GetFeatureName" &&
            method.GetParameters().Length == 1 &&
            method.GetParameters()[0].ParameterType == typeof(int)
        );
        string fixedWidthFallback = requestedName.Substring(0, 15);
        int match = -1;
        int matchCount = 0;
        int featureCount = Convert.ToInt32(count.Invoke(layer, null));
        for (int index = 0; index < featureCount; index++)
        {
            string candidate = Convert.ToString(
                featureName.Invoke(layer, new object[] { index })
            ).TrimEnd('\0', ' ');
            if (candidate != fixedWidthFallback)
                continue;
            match = index;
            matchCount++;
        }
    ...
    public static int Main(string[] args)
    {
        ...
        try
        {
            ...
            int fid = ResolveFeatureId(d2FlowArea, meshName);
            if (fid < 0)
