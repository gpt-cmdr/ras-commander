...
def _comparison_path(value: Union[str, Path]) -> Path:
    """Canonicalize Windows extended/mapped paths for ancestry checks only.

    HEC-RAS must generally receive drive-letter paths, but qualification may
    observe the same long path later as ``\\?\\R:\\...`` or
    ``\\?\\UNC\\server\\share\\...``.  Strip the extended prefix before
    resolving so a mapped drive and its UNC spelling compare as one location.
    """
    text = str(value)
    ...
    try:
        ...
        resolved = Path(text).absolute()
    normalized = str(resolved)
    if normalized.startswith("\\\\?\\UNC\\"):
        normalized = "\\\\" + normalized[8:]
    elif normalized.startswith("\\\\?\\"):
        normalized = normalized[4:]
    return Path(os.path.normcase(normalized))
...
def _project_inventory(project: RasPrj) -> Dict[str, Any]:
    return {
        ...
        "rasmap_loaded": rasmap_dataframe_is_usable(
            getattr(project, "rasmap_df", None)
        ),
...
def project_clone(
    ...
    marker: str = "RASQ Clone",
    **_: Any,
) -> Dict[str, Any]:
    """Clone the staged project and a plan, then reopen without mutating source."""
    from .RasPlan import RasPlan

    source_folder = RasUtils.safe_resolve(Path(str(context["project_folder"])))
    source_before = RasQualification.project_tree_fingerprint(source_folder)
    source_project = _initialize(context)
    plan_number = _plan_number(source_project, source_plan_number)
    stage = RasQualification.stage_project(
        source_folder,
        context["workspace_root"],
        task_id=f"{context['fixture']['id']}-project-clone",
    )
    clone_context = dict(context)
    clone_context.update(
        {
            "project_folder": stage["destination"],
            "project_file": stage["project_file"],
        }
    )
    clone_project = _initialize(clone_context)
    before_inventory = _project_inventory(clone_project)
    clone_marker = str(marker)[:24]
    new_plan_number = RasPlan.clone_plan(
        plan_number,
        new_shortid=clone_marker,
        new_title="Qualification project clone",
        description="Project and plan clone qualification",
        ras_object=clone_project,
    )
    reopened = _initialize(clone_context)
    after_inventory = _project_inventory(reopened)
    new_plan_path = RasPlan.get_plan_path(new_plan_number, ras_object=reopened)
    selected = reopened.plan_df[
        reopened.plan_df["plan_number"].astype(str).str.zfill(2)
        == str(new_plan_number).zfill(2)
    ]
    short_identifier_exact = bool(
        len(selected) == 1
        and str(selected.iloc[0].get("Short Identifier", "")).strip()
        == clone_marker
    )
    source_after = RasQualification.project_tree_fingerprint(source_folder)
    destination_after = RasQualification.project_tree_fingerprint(
        stage["destination"]
    )
    checks = {
        "initial_clone_content_matches": stage["content_matches"] is True,
        "clone_path_distinct": RasUtils.safe_resolve(Path(stage["destination"]))
        != source_folder,
        "source_project_immutable": source_before == source_after,
        "plan_count_incremented": after_inventory["plan_count"]
        == before_inventory["plan_count"] + 1,
        "new_plan_exists": bool(new_plan_path and Path(new_plan_path).is_file()),
        "short_identifier_exact": short_identifier_exact,
        "clone_changed_only_after_plan_clone": destination_after
        != stage["destination_fingerprint"],
    }
    evidence = {
        "checks": checks,
        "stage": stage,
        "source_plan_number": plan_number,
        "new_plan_number": str(new_plan_number).zfill(2),
        "new_plan": _file_record(Path(new_plan_path)) if new_plan_path else None,
        "clone_marker_sha256": hashlib.sha256(clone_marker.encode("utf-8")).hexdigest(),
        "inventory_before_plan_clone": before_inventory,
        "inventory_after_plan_clone": after_inventory,
        "source_fingerprint_before": source_before,
        "source_fingerprint_after": source_after,
        "destination_fingerprint_after": destination_after,
    }
...
def path_variant_open(
    ...
    compute_plan_number: Optional[Union[str, int]] = None,
    num_cores: Optional[int] = None,
    timeout_sec: int = 1800,
    compute_environment: Optional[Mapping[str, Any]] = None,
    require_product_execution: Optional[bool] = None,
    **_: Any,
) -> Dict[str, Any]:
    """Clone/open a path variant and optionally execute HEC-RAS there."""
    ...
    production_profile = str(context.get("executor_profile", "")) in {
        "windows_native",
        "linux_wine_windows_ras",
    }
    product_required = (
        production_profile
        if require_product_execution is None
        else bool(require_product_execution)
    )
    product_execution = None
    if compute_plan_number is not None:
        from .RasCmdr import RasCmdr
        from .RasPlan import RasPlan

        number = _plan_number(opened, compute_plan_number)
        ...
        plan_path = RasPlan.get_plan_path(number, ras_object=opened)
        if plan_path is None:
            raise FileNotFoundError(
                f"Path-variant computation plan {number} disappeared"
            )
        results = RasQualification.results_receipt(Path(str(plan_path) + ".hdf"))
        compute_result_record = _object_dict(compute_result)
        execution_details = compute_result_record.get("execution_details") or {}
        shadow_mode = execution_details.get("mode") == "long_path_execution_shadow"
        source_project_folder = execution_details.get("source_project_folder")
        source_under_variant = bool(
            not shadow_mode
            or (
                source_project_folder
                and _comparison_path(source_project_folder).is_relative_to(
                    _comparison_path(stage["destination"])
                )
            )
        )
        shadow_synchronized = bool(
            not shadow_mode
            or (
                execution_details.get("synchronized") is True
                and execution_details.get("shadow_removed") is True
            )
        )
        project_file_under_variant = bool(
            _comparison_path(opened.prj_file).is_relative_to(
                _comparison_path(stage["destination"])
            )
        )
        product_execution = {
            "plan_number": number,
            "compute_result": compute_result_record,
            "results": results,
            "project_file": str(opened.prj_file),
            "compute_environment": _json_value(compute_environment or {}),
            "project_file_under_variant": project_file_under_variant,
            "long_path_execution_shadow": shadow_mode,
            "shadow_source_under_variant": source_under_variant,
            "shadow_synchronized_and_removed": shadow_synchronized,
            "successful": bool(
                compute_result
                and results["successful"]
                and project_file_under_variant
                and source_under_variant
                and shadow_synchronized
            ),
        }
    checks = {
        "clone_content_matches": stage["content_matches"] is True,
        "project_reopened": evidence["inventory"]["plan_count"] > 0,
        "space_path_created": requested != "spaces" or " " in stage["destination"],
        "minimum_long_path_created": (
            requested != "long" or stage["path_length"] >= minimum_long_path
        ),
        "product_execution_configured_when_required": (
            not product_required or product_execution is not None
        ),
        "product_execution_successful": (
            not product_required
            or bool(product_execution and product_execution["successful"])
        ),
        "product_used_variant_project": (
            not product_required
            or bool(
                product_execution
                and product_execution["project_file_under_variant"]
            )
        ),
    }
    evidence["product_execution_required"] = product_required
    evidence["product_execution"] = product_execution
...
def projection_select(
    ...
    **_: Any,
    ...
    project = _initialize(context)
    rasmap_path = expected_rasmap_path(project.project_folder, project.project_name)
    ...
    reopened = _initialize(context)
    selected = None
    if rasmap_dataframe_is_usable(reopened.rasmap_df):
...
def _mesh_breakline_create(
    ...
    *,
    ...
    cell_delta = None
    final_mesh_topology: Dict[str, Any] = {}
    ...
    evidence = {
        ...
        "expected_face_count": expected_face_count,
        "cell_count_delta": cell_delta,
        "mesh_count_checks": checks,
        "final_mesh_topology": final_mesh_topology or None,
        "geometry_fingerprint": receipt["geometry_fingerprint"],
...
def boundary_associate(
    ...
    additional_lines: Optional[Sequence[Mapping[str, Any]]] = None,
    associate_additional_lines: bool = False,
    ...
    **_: Any,
    ...
    geom_path = _geometry_text(context, project, geometry)
    line_spec = dict(line)
    companion_specs = [dict(item) for item in (additional_lines or [])]
    write_result = GeomBcLines.add_bc_lines(
        geom_path, [line_spec, *companion_specs], replace_existing=True
    )
    if unsteady is None:
    ...
    if associate_additional_lines:
        for companion in companion_specs:
            companion_location = RasUnsteady.ensure_2d_boundary_location(
                unsteady_path,
                area_2d=str(companion["storage_area"]),
                bc_line=str(companion["name"]),
                geometry_file=geom_path,
                ras_object=project,
            )
            companion_attached = RasUnsteady.set_normal_depth_boundary(
                unsteady_path,
                friction_slope=float(friction_slope),
                area_2d=str(companion["storage_area"]),
                bc_line=str(companion["name"]),
            ...
            additional_attachments.append(
                {
                    "storage_area": str(companion["storage_area"]),
                    "name": str(companion["name"]),
                    "location": _json_value(companion_location),
                    "setter_result": _json_value(companion_attached),
                    "passed": bool(companion_location and companion_attached),
                }
            )
    authored = [
        item
    ...
    authored_by_name = {
        (str(item.get("storage_area")), str(item.get("name"))): item
        for item in GeomBcLines.get_bc_lines(geom_path)
    }
    companion_checks = []
    for companion in companion_specs:
        key = (str(companion["storage_area"]), str(companion["name"]))
        persisted = authored_by_name.get(key)
        expected_companion_coordinates = [
            (float(point[0]), float(point[1]))
            for point in companion["coordinates"]
        ]
        actual_companion_coordinates = (
            list(persisted.get("coordinates") or [])
        ...
        companion_checks.append(
            {
                "storage_area": key[0],
                "name": key[1],
                "coordinate_count": len(actual_companion_coordinates),
                "coordinate_max_abs_error": max(errors) if errors else None,
                "content_exact": bool(
                    persisted is not None
                    and len(actual_companion_coordinates)
                    == len(expected_companion_coordinates)
                    and all(error <= coordinate_tolerance for error in errors)
                ),
            }
        )
    additional_lines_content_exact = all(
        item["content_exact"] for item in companion_checks
    )
    reopened = _initialize(context)
    ...
    additional_associations_exact = True
    if associate_additional_lines:
        for companion in companion_specs:
            companion_rows = matches[
                (matches["area_2d"].astype(str) == str(companion["storage_area"]))
                & (matches["bc_line_name"].astype(str) == str(companion["name"]))
            ]
            companion_slope = (
                companion_rows.iloc[0].get("friction_slope_value")
                if len(companion_rows) == 1
                else None
            )
            additional_associations_exact = bool(
                additional_associations_exact
                and len(companion_rows) == 1
                and str(companion_rows.iloc[0].get("bc_type")) == "Normal Depth"
                and companion_slope is not None
                and math.isclose(
                    float(companion_slope),
                    float(friction_slope),
                    rel_tol=0.0,
                    abs_tol=1e-12,
                )
    ...
    evidence = {
        ...
        "additional_lines": companion_checks,
        "additional_lines_content_exact": additional_lines_content_exact,
        ...
        "associate_additional_lines": bool(associate_additional_lines),
        "additional_attachments": additional_attachments,
        "additional_associations_exact": additional_associations_exact,
    ...
    return {
        "passed": bool(
            ...
            and additional_lines_content_exact
            and len(selected) == 1
            and boundary_type_exact
            and friction_slope_exact
            and additional_associations_exact
            and all(item["passed"] for item in additional_attachments)
        ),
        "evidence": evidence,
    }
...
def boundary_conflict_repair(
    ...
    managed_host_attempt_timeout_seconds: Optional[float] = None,
    managed_host_persistence_mode: str = "auto",
    conflict_seed_line: Optional[Mapping[str, Any]] = None,
    **mesh_parameters: Any,
) -> Dict[str, Any]:
    """Repair BC conflicts in text, regenerate, and verify a fresh HDF."""
    from .geom import GeomMesh
    from .geom import GeomBcLines
    from .geom.GeomMesh import (
        _bc_area_records,
        _synchronize_persisted_hdf_mtime,
        _write_global_bc_coordinates,
    )
    ...
    seed_evidence = None
    if conflict_seed_line is not None:
        import h5py
        import numpy as np
        from shapely.geometry import LineString as ShapelyLine

        seed = dict(conflict_seed_line)
        seed_key = (str(seed["storage_area"]), str(seed["name"]))
        seed_coordinates = [
            (float(point[0]), float(point[1]))
            for point in seed["coordinates"]
        ]
        if len(seed_coordinates) < 2:
            raise ValueError("conflict_seed_line requires at least two coordinates")
        before_hdf_lines = hdf_lines(hdf_path)
        if seed_key not in before_hdf_lines:
            raise ValueError(
                "conflict_seed_line must target an existing compiled BC line: "
                f"{seed_key[0]}/{seed_key[1]}"
            )
        text_seed_write = GeomBcLines.add_bc_lines(
            geom_path,
            [
                {
                    "storage_area": seed_key[0],
                    "name": seed_key[1],
        ...
        with h5py.File(hdf_path, "r") as handle:
            area_records = _bc_area_records(handle, ShapelyLine)
        target_area = next(
            (
                item
                for item in area_records
                if item["flow_area_name"] == seed_key[0]
                and any(bc["name"] == seed_key[1] for bc in item["bcs"])
            ),
            None,
        )
        if target_area is None:
            raise ValueError(
                "conflict_seed_line target was not found in normalized HDF records"
            )
        target_area["bc_coords"] = {
            bc["name"]: (
                seed_coordinates
                if bc["name"] == seed_key[1]
                else list(bc["geom"].coords)
                if bc.get("geom") is not None
                else []
            )
            for bc in target_area["bcs"]
        }
        with h5py.File(hdf_path, "r+") as handle:
            if target_area["storage_mode"] == "global_polyline_table":
                _write_global_bc_coordinates(handle, [target_area])
            else:
                bc_path = (
                    f'{target_area["area_group_path"]}/BC Lines/{seed_key[1]}'
                )
                bc_group = handle[bc_path]
                if "Coordinates" in bc_group:
                    del bc_group["Coordinates"]
                bc_group.create_dataset(
                    "Coordinates",
                    data=np.asarray(seed_coordinates, dtype=np.float64),
                )
        timestamp_sync = _synchronize_persisted_hdf_mtime(geom_path, hdf_path)
        seeded_text_lines = text_lines(geom_path)
        seeded_hdf_lines = hdf_lines(hdf_path)
        seed_evidence = {
            "storage_area": seed_key[0],
            "name": seed_key[1],
            "coordinates": seed_coordinates,
            "text_write": text_seed_write,
            "text_matches": coordinates_match(
                seeded_text_lines.get(seed_key), seed_coordinates
            ),
            "hdf_matches": coordinates_match(
                seeded_hdf_lines.get(seed_key), seed_coordinates
            ),
            "timestamp_sync": timestamp_sync,
        }
        if not (
            seed_evidence["text_matches"]
            and seed_evidence["hdf_matches"]
            and timestamp_sync.get("verified") is True
        ):
            raise RuntimeError("conflict_seed_line did not persist exactly")
    ...
    evidence = {
        ...
        "coordinate_abs_tolerance": coordinate_tolerance,
        "conflict_seed_line": seed_evidence,
    }
    returned = {
        "passed": passed,
        "evidence": evidence,
...
def restart_recovery(
    context: Mapping[str, Any],
    restart_plan_number: Optional[Union[str, int]] = None,
    restart_filename: Optional[Union[str, Path]] = None,
    baseline_plan_number: Optional[Union[str, int]] = None,
    prepare_from_plan_number: Optional[Union[str, int]] = None,
    restart_save_datetime: Optional[Union[str, Sequence[str]]] = None,
    warm_start_begin: Optional[str] = None,
    warm_start_end: Optional[str] = None,
    boundary_fixed_start_datetime: Optional[str] = None,
    source_plan_shortid: str = "RST_Source",
    warm_plan_shortid: str = "Warm_Start",
    source_plan_title: str = "Restart output source",
    warm_plan_title: str = "Warm-start continuation",
    series: Optional[Mapping[str, Mapping[str, Any]]] = None,
    series_tolerances: Optional[Mapping[str, Mapping[str, Any]]] = None,
    max_volume_error_difference_percent: Optional[float] = None,
    initial_state_max_abs_tolerance: float = 0.01,
    initial_state_min_abs_signal: float = 0.5,
    timeout_sec: int = 3600,
    num_cores: Optional[int] = None,
    compute_environment: Optional[Mapping[str, Any]] = None,
    **_: Any,
) -> Dict[str, Any]:
    """Create or run a restart fixture and prove hydraulic-state continuity."""
    from .RasCmdr import RasCmdr
    from .RasPlan import RasPlan
    from .RasUnsteady import RasUnsteady
    ...
    def _normalized_variable(specification: Mapping[str, Any]) -> str:
        return re.sub(
            r"[^a-z0-9]+",
            "",
            str(specification.get("variable", "")).lower(),
        )
    ...
    has_hydrograph = any(
        ...
        and (
            ...
            or (
                str(specification.get("kind", "")).strip().lower()
                == "cross_section"
                and _normalized_variable(specification) == "flow"
            )
        )
    ...
    has_wse = any(
        isinstance(specification, Mapping)
        and str(specification.get("kind", "")).strip().lower()
        in {"mesh_cells", "cross_section"}
        and _normalized_variable(specification) == "watersurface"
        for specification in series_specs.values()
    )
    if not has_hydrograph or not has_wse:
        raise ValueError(
            "restart recovery requires an explicit hydrograph and WSE series"
        )
    ...
    state_match_limit = float(initial_state_max_abs_tolerance)
    state_signal_minimum = float(initial_state_min_abs_signal)
    if not math.isfinite(state_match_limit) or state_match_limit < 0:
        raise ValueError(
            "initial_state_max_abs_tolerance must be finite and non-negative"
        )
    if not math.isfinite(state_signal_minimum) or state_signal_minimum < 0:
        raise ValueError(
            "initial_state_min_abs_signal must be finite and non-negative"
        )

    prepared: Optional[Dict[str, Any]] = None
    if prepare_from_plan_number is not None:
        if restart_plan_number is not None or restart_filename is not None:
            raise ValueError(
                "Prepared restart mode derives restart_plan_number and "
                "restart_filename; do not configure them explicitly"
            )
        required_preparation = {
            "restart_save_datetime": restart_save_datetime,
            "warm_start_begin": warm_start_begin,
            "warm_start_end": warm_start_end,
            "boundary_fixed_start_datetime": boundary_fixed_start_datetime,
        }
        missing_preparation = [
            name for name, value in required_preparation.items() if value is None
        ]
        if missing_preparation:
            raise ValueError(
                "Prepared restart mode requires: "
                + ", ".join(missing_preparation)
            )
        template_number = _plan_number(project, prepare_from_plan_number)
        source_number = RasPlan.clone_plan(
            template_number,
            new_shortid=str(source_plan_shortid),
            new_title=str(source_plan_title),
            ras_object=project,
        )
        if not RasPlan.set_restart_output_settings(
            source_number,
            save_datetime=restart_save_datetime,
            write_at_sim_end=False,
            ras_object=project,
        ):
            raise RuntimeError("Could not configure restart-output source plan")
        source_output_settings = RasPlan.get_restart_output_settings(
            source_number,
            ras_object=project,
        )
        expected_name = source_output_settings.get("expected_filename")
        if not expected_name:
            raise RuntimeError("Restart-output settings did not derive an output filename")
        restart_path = _project_path(context, str(expected_name))
        restart_existed_before_source_compute = restart_path.is_file()
        restart_path.unlink(missing_ok=True)
        source_compute_result = RasCmdr.compute_plan(
            source_number,
            ras_object=project,
            force_rerun=True,
            verify=True,
        ...
        if not source_compute_result or not restart_path.is_file():
            return {
                "passed": False,
                "evidence": {
                    "preparation_mode": "clone_compute_and_continue",
                    "template_plan_number": template_number,
                    "source_plan_number": str(source_number).zfill(2),
                    "source_output_settings": _json_value(source_output_settings),
                    "source_compute_result": _object_dict(source_compute_result),
                    "restart_existed_before_source_compute": (
                        restart_existed_before_source_compute
                    ),
                    "restart_file_created": restart_path.is_file(),
                    "expected_restart_file": str(restart_path),
                },
                "diagnostics": {
                    "reason": "Source computation did not create the restart file"
                },
            }

        project = _initialize(context)
        source_row = project.plan_df[
            project.plan_df["plan_number"].astype(str).str.zfill(2)
            == str(source_number).zfill(2)
        ]
        if len(source_row) != 1:
            raise RuntimeError("Prepared restart source plan did not reopen uniquely")
        source_unsteady_number = RasUtils.normalize_ras_number(
            source_row.iloc[0]["unsteady_number"]
        )
        warm_unsteady_number = RasPlan.clone_unsteady(
            source_unsteady_number,
            new_title="Warm restart usage",
            ras_object=project,
        )
        RasUnsteady.set_restart_settings(
            warm_unsteady_number,
            use_restart=True,
            restart_filename=restart_path.name,
            ras_object=project,
        )
        boundary_timing_settings = RasUnsteady.set_hydrograph_fixed_start_time(
            warm_unsteady_number,
            use_fixed_start_time=True,
            fixed_start_datetime=boundary_fixed_start_datetime,
            ras_object=project,
        )
        warm_number = RasPlan.clone_plan(
            source_number,
            new_shortid=str(warm_plan_shortid),
            new_title=str(warm_plan_title),
            unsteady_flow=warm_unsteady_number,
            ras_object=project,
        )
        RasPlan.update_simulation_date(
            warm_number,
            pd.Timestamp(str(warm_start_begin)).to_pydatetime(),
            pd.Timestamp(str(warm_start_end)).to_pydatetime(),
            ras_object=project,
        )
        if not RasPlan.set_restart_output_settings(
            warm_number,
            ...
            raise RuntimeError("Could not disable restart output on continuation plan")
        warm_output_settings = RasPlan.get_restart_output_settings(
            warm_number,
            ras_object=project,
        )
        baseline_plan_number = source_number
        restart_plan_number = warm_number
        restart_filename = restart_path.name
        prepared = {
            "mode": "clone_compute_and_continue",
            "template_plan_number": template_number,
            "source_plan_number": str(source_number).zfill(2),
            "source_unsteady_number": source_unsteady_number,
            "warm_plan_number": str(warm_number).zfill(2),
            "warm_unsteady_number": str(warm_unsteady_number).zfill(2),
            "source_output_settings": _json_value(source_output_settings),
            "warm_output_settings": _json_value(warm_output_settings),
            "boundary_timing_settings": _json_value(boundary_timing_settings),
            "source_compute_result": _object_dict(source_compute_result),
            "compute_environment": _json_value(compute_environment or {}),
            "restart_existed_before_source_compute": (
                restart_existed_before_source_compute
    ...
    if restart_plan_number is None or restart_filename is None:
        raise ValueError(
            "restart_plan_number and restart_filename are required unless "
            "prepare_from_plan_number is configured"
        )
    ...
    initial_state_proof = None
    initial_state_matches = True
    if prepared is not None:
        cross_section_wse = next(
            (
                (name, specification)
                for name, specification in series_specs.items()
        ...
        if cross_section_wse is None:
            raise ValueError(
                "Prepared restart mode requires an explicit cross-section WSE series "
                "to prove the loaded initial state"
            )
        proof_name, proof_specification = cross_section_wse
        full_specification = dict(proof_specification)
        full_specification.pop("start_time", None)
        full_specification.pop("end_time", None)
        source_full = RasQualification.extract_result_series(
            baseline_hdf,
            {str(proof_name): full_specification},
            ras_object=project,
        )[str(proof_name)]
        restart_full = RasQualification.extract_result_series(
            Path(str(plan_path) + ".hdf"),
            {str(proof_name): full_specification},
            ras_object=project,
        )[str(proof_name)]
        value_column = str(source_full["value_columns"][0])
        source_frame = pd.DataFrame(source_full["records"])
        restart_frame = pd.DataFrame(restart_full["records"])
        source_times = pd.to_datetime(source_frame["time"], errors="raise")
        restart_times = pd.to_datetime(restart_frame["time"], errors="raise")
        restart_timestamp = pd.Timestamp(str(warm_start_begin))
        source_restart_index = int(
            (source_times - restart_timestamp).abs().to_numpy().argmin()
        )
        restart_initial_index = int(restart_times.to_numpy().argmin())
        source_cold_value = float(source_frame.iloc[0][value_column])
        source_restart_value = float(
            source_frame.iloc[source_restart_index][value_column]
        )
        restart_initial_value = float(
            restart_frame.iloc[restart_initial_index][value_column]
        )
        loaded_state_abs_error = abs(restart_initial_value - source_restart_value)
        restart_state_signal = abs(restart_initial_value - source_cold_value)
        source_time_abs_error_seconds = abs(
            (source_times.iloc[source_restart_index] - restart_timestamp).total_seconds()
        )
        restart_time_abs_error_seconds = abs(
            (restart_times.iloc[restart_initial_index] - restart_timestamp).total_seconds()
        )
        initial_state_checks = {
            "source_restart_timestamp_exact": source_time_abs_error_seconds == 0,
            "restart_initial_timestamp_exact": restart_time_abs_error_seconds == 0,
            "loaded_state_matches_source_restart": (
                loaded_state_abs_error <= state_match_limit
            ),
            "loaded_state_differs_from_cold_start": (
                restart_state_signal >= state_signal_minimum
            ),
        }
        initial_state_matches = bool(all(initial_state_checks.values()))
        initial_state_proof = {
            "series_name": str(proof_name),
            "river": source_full.get("river"),
            "reach": source_full.get("reach"),
            "station": source_full.get("station"),
            "variable": source_full.get("variable"),
            "value_column": value_column,
            "restart_timestamp": restart_timestamp.isoformat(),
            "source_cold_start_value": source_cold_value,
            "source_at_restart_value": source_restart_value,
            "restart_initial_value": restart_initial_value,
            "loaded_state_abs_error": loaded_state_abs_error,
            "loaded_state_max_abs_tolerance": state_match_limit,
            "restart_state_signal": restart_state_signal,
            "restart_state_min_abs_signal": state_signal_minimum,
            "source_time_abs_error_seconds": source_time_abs_error_seconds,
            "restart_time_abs_error_seconds": restart_time_abs_error_seconds,
            "checks": initial_state_checks,
        }
    evidence = {
        ...
        "compute_environment": _json_value(compute_environment or {}),
        "initial_state_proof": initial_state_proof,
    }
    return {
        "passed": bool(
            result
...
def failed_run_diagnostics(
    context: Mapping[str, Any],
    failing_plan_number: Optional[Union[str, int]] = None,
    expected_patterns: Sequence[str] = (),
    prepare_from_plan_number: Optional[Union[str, int]] = None,
    failure_mode: str = "missing_restart_file",
    missing_restart_filename: str = "qualification-intentionally-missing.rst",
    solver_abort_threshold: float = 0.001,
    failing_plan_shortid: str = "Expected_Failure",
    timeout_sec: int = 900,
    compute_environment: Optional[Mapping[str, Any]] = None,
    **_: Any,
) -> Dict[str, Any]:
    ...
    preparation = None
    if prepare_from_plan_number is not None:
        if failing_plan_number is not None:
            raise ValueError(
                "Prepared failed-run mode derives failing_plan_number; do not "
                "configure it explicitly"
            )
        normalized_mode = str(failure_mode).strip().lower()
        if normalized_mode not in {
            "missing_restart_file",
            "solver_abort_threshold",
        }:
            raise ValueError(
                "Supported failed-run preparation modes are 'missing_restart_file' "
                "and 'solver_abort_threshold'"
            )
        template_number = _plan_number(project, prepare_from_plan_number)
        template_row = project.plan_df[
            project.plan_df["plan_number"].astype(str).str.zfill(2)
            == template_number
        ]
        if len(template_row) != 1:
            raise RuntimeError("Failed-run template plan did not resolve uniquely")
        template_unsteady = RasUtils.normalize_ras_number(
            template_row.iloc[0]["unsteady_number"]
        )
        failing_unsteady = template_unsteady
        missing_restart_path = None
        if normalized_mode == "missing_restart_file":
            failing_unsteady = RasPlan.clone_unsteady(
                template_unsteady,
                new_title="Intentional missing restart",
                ras_object=project,
            )
            missing_restart_path = _project_path(context, missing_restart_filename)
            missing_restart_path.unlink(missing_ok=True)
            RasUnsteady.set_restart_settings(
                failing_unsteady,
                use_restart=True,
                restart_filename=missing_restart_path.name,
                ras_object=project,
            )
        failing_plan_number = RasPlan.clone_plan(
            template_number,
            new_shortid=str(failing_plan_shortid),
            new_title="Intentional expected failure",
            unsteady_flow=failing_unsteady,
            ras_object=project,
        )
        threshold_value = None
        threshold_parameter_persisted = None
        if normalized_mode == "solver_abort_threshold":
            threshold_value = float(solver_abort_threshold)
            if not math.isfinite(threshold_value) or threshold_value <= 0:
                raise ValueError(
                    "solver_abort_threshold must be finite and greater than zero"
                )
            failing_plan_path = RasPlan.get_plan_path(
                failing_plan_number,
                ras_object=project,
            )
            if failing_plan_path is None:
                raise FileNotFoundError("Prepared solver-abort plan disappeared")

            def _set_abort_threshold(lines: List[str], value: float) -> List[str]:
                key = "UNET DZMax Abort"
                matches = [
                    index
                    for index, line in enumerate(lines)
                ...
                if len(matches) != 1:
                    raise RuntimeError(
                        f"Expected exactly one {key!r} line; found {len(matches)}"
                    )
                lines[matches[0]] = f"{key}= {value:.12g} \n"
                return lines

            RasUtils.update_file(
                Path(failing_plan_path),
                _set_abort_threshold,
                threshold_value,
            )
            threshold_lines = [
                line.strip()
                for line in Path(failing_plan_path).read_text(
                    encoding="utf-8",
                    errors="replace",
                ).splitlines()
                if line.split("=", 1)[0].strip() == "UNET DZMax Abort"
            ]
            threshold_parameter_persisted = bool(
                threshold_lines
                and math.isclose(
                    float(threshold_lines[0].split("=", 1)[1].strip()),
                    threshold_value,
                    rel_tol=0.0,
                    abs_tol=0.0,
                )
        ...
        preparation = {
            ...
            "template_plan_number": template_number,
            "template_unsteady_number": template_unsteady,
            "failing_plan_number": str(failing_plan_number).zfill(2),
            "failing_unsteady_number": str(failing_unsteady).zfill(2),
            "missing_restart_filename": (
                missing_restart_path.name if missing_restart_path is not None else None
            ),
            "missing_restart_absent_before_compute": (
                not missing_restart_path.exists()
                if missing_restart_path is not None
                else None
            ),
            "solver_abort_threshold": threshold_value,
            "solver_abort_threshold_persisted": threshold_parameter_persisted,
        }
    if failing_plan_number is None:
        raise ValueError(
            "failing_plan_number is required unless prepare_from_plan_number is configured"
        )
    ...
    plan_path = Path(plan_path)
    detailed_logging_enabled = BcoMonitor.enable_detailed_logging(plan_path)
    caught = None
    result = None
    try:
        result = RasCmdr.compute_plan(
    ...
    hdf_path = Path(str(plan_path) + ".hdf")
    vendor_message_parts = []
    message_sources = []
    if hdf_path.is_file():
        hdf_messages = HdfResultsPlan.get_compute_messages_hdf_only(hdf_path) or ""
        if hdf_messages:
            vendor_message_parts.append(hdf_messages)
            message_sources.append(
                {
                    "kind": "plan_hdf_compute_messages",
                    "path": str(hdf_path),
                    "message_length": len(hdf_messages),
                    "message_sha256": hashlib.sha256(
                        hdf_messages.encode("utf-8")
                    ).hexdigest(),
                }
            )
    project_name = str(project.project_name)
    message_candidates = [
        plan_path.parent / f"{project_name}.bco{number}",
        plan_path.parent / f"{project_name}.p{number}.computeMsgs.txt",
        plan_path.parent / f"{project_name}.p{number}.comp_msgs.txt",
    ]
    seen_candidates = set()
    message_files = []
    for candidate in message_candidates:
        ...
        if text:
            vendor_message_parts.append(text)
        receipt = _file_record(candidate)
        receipt["message_length"] = len(text)
        message_files.append(receipt)
        message_sources.append({"kind": "vendor_message_file", **receipt})
    vendor_messages = "\n".join(vendor_message_parts)
    ...
    vendor_parsed = ResultsParser.parse_compute_messages(vendor_messages)
    computation_failed = bool(
        caught is not None or not bool(result) or vendor_parsed.get("has_errors")
    )
    preparation_valid = bool(
        preparation is None
        or (
            preparation.get("mode") == "missing_restart_file"
            and preparation.get("missing_restart_absent_before_compute") is True
        )
        or (
            preparation.get("mode") == "solver_abort_threshold"
            and preparation.get("solver_abort_threshold_persisted") is True
        )
    )
    evidence = {
        ...
        "vendor_message_length": len(vendor_messages),
        "vendor_message_sha256": hashlib.sha256(
            vendor_messages.encode("utf-8")
        ).hexdigest(),
        "message_sources": message_sources,
        "message_files": message_files,
        "expected_pattern_matches": matches,
        "parsed_diagnostics": _json_value(parsed),
        "vendor_parsed_diagnostics": _json_value(vendor_parsed),
