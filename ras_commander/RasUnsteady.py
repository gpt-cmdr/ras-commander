...
class RasUnsteady:
    ...
    def _read_hdf_precip_attrs(
        ...
        try:
            ...
            logger.warning(
                f"Could not read precipitation attributes from {hdf_path.name}: {e}"
            )
...
# Need to add examples
    ...
    def configure_gridded_dss_precipitation(
        ...
        if not unsteady_path.exists():
            raise FileNotFoundError(f"Unsteady flow file not found: {unsteady_path}")
        # Path.resolve() turns mapped drives into UNC paths, which HEC-RAS cannot read,
        # and this path feeds the relative DSS filename written into the .u##.
        from .RasUtils import RasUtils
