...
- Gridded precipitation payloads: `RasPrecipHdf` writes HEC-RAS's `Imported Raster Data` payload (`Values`, `Values (Vertical)`) into an unsteady `.u##.hdf` in the native layout (HEC-RAS chunk rules, gzip 1, NaN fill, 14 attributes + optional `NoData`). `write_gridded_precip_raster()` has mandatory `units`, `overwrite=False`, `dry_run`, and requires a `Met BC=Precipitation|` block in the `.u##` because HEC-RAS destroys the payload on the next save otherwise; `convert_to_cumulative()` (`value_type` rate/amount/cumulative, `first_timestep_hours` mirrors the import dialog), `orient_north_up()`, `get_grid_from_coords()`, `get_values_chunks()` / `get_vertical_chunks()`, `normalize_units()` / `infer_depth_units()` / `validate_data_type_units()`. `RasUnsteady.set_gridded_precipitation()` is the NetCDF facade (`units`, `value_type`, `first_timestep_hours`, `ratio`). Units are honored by HEC-RAS: a mislabelled payload scales precipitation by 25.4. Precipitation only.
