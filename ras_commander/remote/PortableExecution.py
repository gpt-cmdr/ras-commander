"""Execute one portable, one-core steady-plan request.

This is the common in-container entry point for Docker and Slurm/Apptainer.
It never computes in the source project.  A complete project tree is copied to
the request's isolated output directory before :class:`RasCmdr` is initialized.
"""
...
def _argv_invocation(ras_exe_path, project_path, plan_path):
    """Portable override for the legacy shell-string compute branch."""
    return [str(ras_exe_path), "-c", str(project_path), str(plan_path)], False
...
def execute_request(request_path: Union[str, Path]) -> RasExecutionReceipt:
    ...
    try:
        ...
        original_invocation = RasCmdr._compute_process_invocation
        RasCmdr._compute_process_invocation = staticmethod(_argv_invocation)
        try:
            result = RasCmdr.compute_plan(
                request.plan_number,
                ras_object=ras_object,
                # Cleanup was performed explicitly and verified above because
                # this RasCmdr version logs and swallows clear failures.
                clear_geompre=False,
                force_geompre=False,
                force_rerun=True,
                num_cores=1,
            ...
            RasCmdr._compute_process_invocation = staticmethod(original_invocation)
