        ...
        *,
        ...
        if route_normalized not in valid_routes:
            raise ValueError(
                ...
                "ras_commander, native, or translated_netcdf_with_native_hdf"
            )
...
class PrecipCapabilities:
    ...
    def for_version(version: object) -> GriddedPrecipitationCapabilities:
        ...
        normalized = normalize_program_version(version)
        ...
        prerelease = "beta" in str(version).lower()
        if release[0] < 6:
        ...
        geotiff_qualification: QualificationStatus = (
            "qualified_windows_and_wine"
            if normalized == "6.6"
            else ("historical_beta" if prerelease else "documented")
        )
        ...
        return GriddedPrecipitationCapabilities(
            ...
            native_netcdf_qualification=netcdf_qualification,
    ...
    def resolve(
        *,
        ...
        candidate = version
        if candidate is None and ras_object is not None:
            candidate = getattr(ras_object, "ras_version", None)
        if candidate is None:
            raise ValueError(
                "A HEC-RAS version is required to evaluate precipitation capabilities"
            )
        return PrecipCapabilities.for_version(candidate)
