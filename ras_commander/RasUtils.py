...
class RasUtils:
    ...
    @staticmethod
    def windows_extended_path(path: Path, *, force: bool = False) -> Path:
        """Return a Windows extended-length path when needed or explicitly forced.

        The returned path is for filesystem operations only. User-facing and
        HEC-RAS paths should retain their normal drive-letter form. ``force``
        is useful when a currently short directory will contain descendants
        whose full paths can exceed ``MAX_PATH``.
        """
        path = Path(path)
        if os.name != "nt":
            return path
        ...
        if value.startswith("\\\\?\\"):
            return Path(value)
        if len(value) < 248 and not force:
            return Path(value)
        if value.startswith("\\\\"):
