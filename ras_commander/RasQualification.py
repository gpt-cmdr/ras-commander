...
class RasQualification:
    ...
    def stage_project(
        ...
        destination_io = RasUtils.windows_extended_path(destination, force=True)
    ...
    def extract_result_series(
        ...
        series. ``cross_section`` specifications select one explicit 1D river,
        reach, and station. Explicit feature locations are required so a parity
        run cannot pass by silently comparing a different spatial sample.
        """
        from .hdf import HdfResultsMesh, HdfResultsXsec
        ...
        for series_name, specification in specifications.items():
            ...
            if kind == "cross_section":
                river = str(specification.get("river", "")).strip()
                reach = str(specification.get("reach", "")).strip()
                station = str(specification.get("station", "")).strip()
                variable = str(specification.get("variable", "")).strip()
                if not river or not reach or not station or not variable:
                    raise ValueError(
                        f"Series {series_name!r} requires river, reach, station, "
                        "and variable"
                    )
                dataset = HdfResultsXsec.get_xsec_timeseries(path)
                normalized_variable = re.sub(r"[^a-z0-9]+", "", variable.lower())
                variable_matches = [
                    str(name)
                    for name in dataset.data_vars
                    if re.sub(r"[^a-z0-9]+", "", str(name).lower())
                    == normalized_variable
                ]
                if len(variable_matches) != 1:
                    raise ValueError(
                        f"Series {series_name!r} variable {variable!r} matched "
                        f"{len(variable_matches)} cross-section result variables"
                    )
                variable_name = variable_matches[0]
                if "cross_section" not in dataset.dims:
                    raise RuntimeError(
                        f"Series {series_name!r} cross-section results have no "
                        "cross_section dimension"
                    )
                required_coordinates = {"River", "Reach", "Station"}
                missing_coordinates = required_coordinates - set(dataset.coords)
                if missing_coordinates:
                    raise RuntimeError(
                        f"Series {series_name!r} is missing cross-section coordinates: "
                        f"{sorted(missing_coordinates)}"
                    )

                def _coordinate_strings(name: str) -> np.ndarray:
                    return np.asarray(
                        [str(value).strip() for value in dataset.coords[name].values],
                        dtype=object,
                    )

                matching = np.flatnonzero(
                    (_coordinate_strings("River") == river)
                    & (_coordinate_strings("Reach") == reach)
                    & (_coordinate_strings("Station") == station)
                )
                if len(matching) != 1:
                    raise ValueError(
                        f"Series {series_name!r} location {river}/{reach}/{station} "
                        f"matched {len(matching)} cross sections"
                    )
                cross_section_index = int(matching[0])
                value_column = str(
                    specification.get(
                        "value_column",
                        "wse"
                        if normalized_variable == "watersurface"
                        else "flow"
                ...
                if not value_column:
                    raise ValueError(
                        f"Series {series_name!r} requires a non-empty value_column"
                    )
                selected = dataset[variable_name].isel(
                    cross_section=cross_section_index
                )
                frame = selected.to_dataframe(name=value_column).reset_index()
                frame = frame[["time", value_column]].copy()
                frame["river"] = river
                frame["reach"] = reach
                ...
                if frame.empty or not finite_values.notna().any():
                    raise RuntimeError(
                        f"Cross-section series {series_name!r} contains no numeric "
                        f"{value_column} values"
                    )
                extracted[str(series_name)] = {
                    "kind": kind,
                    "river": river,
                    ...
                    "start_time": _json_value(specification.get("start_time")),
                    "end_time": _json_value(specification.get("end_time")),
                    "units": _json_value(selected.attrs.get("units")),
                    "value_columns": [value_column],
                    "records": _frame_records(frame),
                }
                continue
            ...
            raise ValueError(
                ...
                "'profile_line_flow', 'mesh_cells', or 'cross_section'"
            )
        return extracted
    ...
    def validate_run_receipt(
        ...
        for name, item in series.items():
            if not isinstance(item, Mapping):
                continue
            is_profile_flow = item.get("kind") == "profile_line_flow"
            is_cross_section_flow = bool(
                item.get("kind") == "cross_section"
                and re.sub(
                    r"[^a-z0-9]+",
                    "",
                    str(item.get("variable", "")).lower(),
                )
                == "flow"
            )
            value_columns = item.get("value_columns") or []
            if (
                (is_profile_flow or is_cross_section_flow)
                and len(value_columns) == 1
                and _records_are_complete(item, str(value_columns[0]))
            ):
                hydrograph_series.append(name)
        wse_series = []
        for name, item in series.items():
            if not isinstance(item, Mapping):
                continue
            value_columns = item.get("value_columns") or []
            normalized_variable = re.sub(
                r"[^a-z0-9]+",
                "",
                str(item.get("variable", "")).lower(),
            )
            is_wse = bool(
                item.get("kind") in {"mesh_cells", "cross_section"}
                and normalized_variable == "watersurface"
            )
    ...
    def consolidate_run_receipts(
        ...
        *,
        ...
        series_sources: Optional[
            Mapping[str, Union[str, Mapping[str, str]]]
        ] = None,
    ) -> Dict[str, Any]:
        """Consolidate focused, content-qualified runs into one strict gate receipt.

        Each required operation is mapped explicitly to a named source. A source
        may be a complete run receipt or a retained worker action result. Run
        receipts may be globally red because unrelated operations were not
        configured, but the selected operation itself must be passed and carry
        evidence. All run-receipt sources must identify the same executor profile
        and HEC-RAS component binaries.
        """
        if not isinstance(sources, Mapping) or not sources:
            raise ValueError("sources must contain at least one named receipt")
        if not str(fixture_id).strip() or not str(fixture_description).strip():
            raise ValueError("fixture_id and fixture_description are required")
        ...
        for alias, source in sources.items():
            ...
            if not name:
                raise ValueError("source aliases must be non-empty")
            if name in loaded:
                raise ValueError(f"duplicate source alias: {name}")
            source_path = None
            if isinstance(source, Mapping):
                payload = _json_value(dict(source))
                encoded = json.dumps(
                    payload,
                    sort_keys=True,
                    separators=(",", ":"),
                ...
                if not source_path.is_file():
                    raise FileNotFoundError(
                        f"Consolidation source not found: {source_path}"
                    )
                payload = RasQualification.read_receipt(source_path)
                source_sha256 = RasQualification.file_sha256(source_path)
            if not isinstance(payload, Mapping):
                raise TypeError(f"Consolidation source {name!r} is not a mapping")
            source_type = (
                "run_receipt"
                if isinstance(payload.get("operations"), list)
                and isinstance(payload.get("installation"), Mapping)
                else "action_result"
            ...
            if source_type == "unknown":
                raise ValueError(
                    f"Consolidation source {name!r} is neither a run receipt "
                    "nor an action result"
                )
            ...
            provenance[name] = {
                ...
                "fixture_id": (
                    (payload.get("fixture") or {}).get("id")
                    if source_type == "run_receipt"
                    else None
                ),
            }

        installation_alias = str(installation_source)
        if installation_alias not in loaded:
            raise KeyError(
                f"installation_source {installation_alias!r} is not in sources"
            )
        installation_receipt = loaded[installation_alias]
        if provenance[installation_alias]["source_type"] != "run_receipt":
            raise ValueError("installation_source must name a complete run receipt")
        profile = str(installation_receipt.get("executor_profile", ""))
        installation = installation_receipt.get("installation") or {}
        if profile not in {item.value for item in ExecutorProfile}:
            raise ValueError("installation_source has an invalid executor profile")

        def _installation_identity(receipt: Mapping[str, Any]) -> Dict[str, Any]:
            item = receipt.get("installation") or {}
            components = item.get("components") or {}
            return {
                "executor_profile": receipt.get("executor_profile"),
                "expected_version": item.get("expected_version"),
                "detected_version": item.get("detected_version"),
                "components": {
                    str(name): {
                        "sha256": component.get("sha256"),
                        "architecture": (component.get("pe") or {}).get(
                            "architecture"
                        ),
                    }
        ...
        reference_identity = _installation_identity(installation_receipt)
        if not reference_identity["components"]:
            raise ValueError("installation_source has no component identity")
        for alias, payload in loaded.items():
            if provenance[alias]["source_type"] != "run_receipt":
                continue
            identity = _installation_identity(payload)
            if identity != reference_identity:
                raise ValueError(
                    f"Run receipt {alias!r} does not match the consolidated "
                    "installation identity"
                )

        required_operation_sources = {
            operation_id
            for operation_id in RasQualification.REQUIRED_OPERATIONS
            if operation_id
            not in RasQualification._PROFILE_NOT_APPLICABLE.get(profile, set())
        }
        configured_operations = set(operation_sources)
        if configured_operations != required_operation_sources:
            missing = sorted(required_operation_sources - configured_operations)
            unexpected = sorted(configured_operations - required_operation_sources)
            raise ValueError(
                "operation_sources must cover required operations exactly; "
                f"missing={missing}, unexpected={unexpected}"
            )

        consolidation_specification = {
            "sources": provenance,
            "operation_sources": dict(operation_sources),
            "artifact_sources": _json_value(dict(artifact_sources or {})),
            "series_sources": _json_value(dict(series_sources or {})),
        }
        ...
        fixture = {
            "id": str(fixture_id),
            "description": str(fixture_description),
            "source_fingerprint": source_fingerprint,
            "component_fixture_ids": sorted(
                {
                    str(item["fixture_id"])
        ...
        consolidated = RasQualification.create_run_receipt(
            profile,
            installation=installation,
            fixture=fixture,
        )

        for operation_id, alias_value in operation_sources.items():
            alias = str(alias_value)
            if alias not in loaded:
                raise KeyError(
                    f"Operation {operation_id} references unknown source {alias!r}"
                )
            payload = loaded[alias]
            if provenance[alias]["source_type"] == "run_receipt":
                matches = [
                    item
                ...
                if len(matches) != 1:
                    raise ValueError(
                        f"Source {alias!r} contains {len(matches)} records for "
                        f"operation {operation_id}"
                    )
                selected = matches[0]
                passed = selected.get("status") == "passed"
                evidence = selected.get("evidence")
            else:
                selected = payload
                passed = selected.get("passed") is True
                evidence = selected.get("evidence")
            if not passed or not isinstance(evidence, Mapping) or not evidence:
                raise ValueError(
                    f"Selected source {alias!r} does not contain passed evidence "
                    f"for operation {operation_id}"
                )
            operation_evidence = dict(evidence)
            operation_evidence["qualification_provenance"] = {
                "source_alias": alias,
                **provenance[alias],
            }
        ...
        def _select_collection_item(
            output_name: str,
            descriptor: Union[str, Mapping[str, str]],
            collection_name: str,
        ) -> tuple[Any, Dict[str, Any]]:
            if isinstance(descriptor, Mapping):
                alias = str(descriptor.get("source", ""))
                source_key = str(descriptor.get("key", output_name))
            else:
                alias = str(descriptor)
                source_key = output_name
            if alias not in loaded:
                raise KeyError(
                    f"{collection_name} {output_name!r} references unknown "
                    f"source {alias!r}"
                )
            collection = loaded[alias].get(collection_name) or {}
            if not isinstance(collection, Mapping) or source_key not in collection:
                raise KeyError(
                    f"Source {alias!r} has no {collection_name}.{source_key}"
                )
        ...
        artifact_provenance = {}
        for name, descriptor in (artifact_sources or {}).items():
            item, item_provenance = _select_collection_item(
                str(name), descriptor, "artifacts"
            )
            consolidated["artifacts"][str(name)] = _json_value(item)
            artifact_provenance[str(name)] = item_provenance
        series_provenance = {}
        for name, descriptor in (series_sources or {}).items():
            item, item_provenance = _select_collection_item(
                str(name), descriptor, "series"
            )
            consolidated["series"][str(name)] = _json_value(item)
            series_provenance[str(name)] = item_provenance

        consolidated["consolidation"] = {
            **consolidation_specification,
            "artifact_provenance": artifact_provenance,
            "series_provenance": series_provenance,
            "installation_identity": reference_identity,
        }
        return consolidated
