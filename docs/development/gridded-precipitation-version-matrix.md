...
The WPC qualification also identified an external-tool limitation independent
of HEC-RAS version: `qpkit 0.1.0` produced the same undersized 2-by-2 DSS grid
for materially different `QPFGridOptions.extents` requests. Notebook 926 avoids
that path by cropping and reopening every GRIB in its native projection before
calling `kit.qpf_dss.write()` without `extents`.
...
| 728 DSS window extension | Model-covering deterministic DSS family copied with an explicit zero tail, then applied through `configure_gridded_dss_precipitation()` | Fully executed under HEC-RAS 7.0: source SHA preserved, 3 source plus 3 appended records, five-hour model window, exact three-hour dry tail, 0.0000002-inch DSS-to-temporary-HDF maximum error, zero flagged BCO diagnostics, 18,066 active final-HDF cells, 0.000005-inch mean error and 0.000693-inch RMSE, positive hydraulic results, and 4 review figures | Retain source-immutability, temporary/final-HDF, zero-tail plateau, hydraulic, and manual-review gates; the helper remains mechanical preparation and does not infer a scientifically appropriate drain-down window |
...
| 914 historical diagnostic | Archived AORC NetCDF direct route plus USGS stage | Fully executed under HEC-RAS 7.0: exact 48-hour source timing, durable native-HDF authoring, 0.000003-inch source-to-temporary and source-to-final maximum errors, 18,066 active final-HDF cells, zero flagged BCO diagnostics, 0.001523% volume error, five review figures, and a timezone-correct USGS comparison | Retain the source/precompute/final-HDF gates and explicit manual review. The gauge comparison is diagnostic, not calibration validation, because the fixture does not include reviewed historical boundary hydrographs, initial conditions, or gate/control histories. |
...
| 917 MRMS | GRIB2 to DSS for spatial source inspection, then intentionally area-averaged boundary hyetographs | Fully executed under HEC-RAS 7.0: 7 of 7 code cells, 61 DSS grids across two archived events (37 Davis and 24 New Orleans), four baseline/event simulations, exact applied boundary totals verified in the final HDF, zero flagged BCO diagnostics, 60 reviewed hydraulic frames, 8 embedded figures, and 6 MP4s; 3 Davis and 30 New Orleans Mapper gaps were transparently filled from final-HDF depth results | Retain as the canonical uniform-boundary hydraulic comparison and never count it as global gridded-meteorology evidence; notebook 924 is the canonical spatial MRMS rain-on-grid qualification |
| 916 HRRR | Archived HRRR GRIB cumulative APCP differenced into 18 hourly native DSS grids | Fully executed under HEC-RAS 7.0 with source/DSS plots, exact DSS-to-temporary-HDF check (0.000002-inch maximum error), active-cell final-HDF comparison, no-rain/forecast hydraulics, mass balance, runtime-message audit, and mapped localized convergence exceptions | Retain as the forecast/DSS canonical example; qualify additional supported version families separately and continue manual review of messages, rainfall maps, and the three localized cells above 0.1-foot maximum water-surface error |
| 915 forecast orchestration | Deterministic cycle manifest, current public-signature audit, timezone conversion, and executed-component dashboard | Fully executed: 6 of 6 code cells, zero stored errors, two review figures, and artifact checks over executed notebooks 916/923/924/926 | Retain as an orchestration/readiness guide. It deliberately performs no live download or HEC-RAS compute and does not count as format/version qualification; component notebooks retain that evidence. |
| 926 WPC | Current WPC GRIB2 cropped in its native projection, translated to DSS, and applied through global gridded meteorology | Fully executed under HEC-RAS 7.0: 11 of 11 code cells, complete 28-grid 26 September 2026 00Z cycle, objective wettest 24-hour window, source-to-DSS regional checks, exact DSS-to-temporary-HDF check (0.0000002-inch maximum error), no-rain/event computes, zero flagged BCO diagnostics, active-cell final-HDF comparison (-0.000229-inch mean error; 0.003105-inch RMSE), hydraulic response, and 5 review figures | Retain the native-projection pre-crop workaround for qpkit 0.1.0, the explicit manual-review requirement, and mapped review of the three localized cells above 0.1-foot maximum WSEL error |
