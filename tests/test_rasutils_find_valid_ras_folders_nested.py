...
so ``include_nested_projects=True`` opts back in to descending.
"""
...
def test_nested_project_pruned_by_default_and_found_when_requested(tmp_path):
    ...
    nested_found = RasUtils.find_valid_ras_folders(
        tmp_path, include_nested_projects=True
    )
...
def test_include_nested_returns_project_info_for_both_levels(tmp_path):
    ...
    info = RasUtils.find_valid_ras_folders(
        tmp_path, return_project_info=True, include_nested_projects=True
    )
