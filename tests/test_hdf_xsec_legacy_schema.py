...
def test_reads_hec_ras_5_separated_cross_section_schema(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(HdfXsec, "_resolve_projection", lambda *_args, **_kwargs: "EPSG:26916")
...
def test_legacy_schema_out_of_bounds_profile_slice_fails_closed(tmp_path):
    ...
    with pytest.raises(ValueError, match="Invalid Station Elevation Info slice"):
        HdfXsec.get_cross_sections(hdf_path)
...
def test_legacy_schema_out_of_bounds_declared_polyline_fails_closed(tmp_path):
    ...
    with pytest.raises(ValueError, match="Invalid Polyline Points slice"):
        HdfXsec.get_cross_sections(hdf_path)
...
def test_reads_legacy_centerline_and_bank_line_schemas(tmp_path, monkeypatch):
    hdf_path = tmp_path / "Legacy.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    monkeypatch.setattr(HdfXsec, "_resolve_projection", lambda *_args, **_kwargs: None)
...
def _add_stale_compound_tables(path: Path, rows: int = 3) -> None:
    """Add a HEC-RAS 5.0.3 hybrid's stale compound tables with a wrong row count."""
    attributes_dtype = np.dtype(
        [
            ("River", "S16"),
            ("Reach", "S16"),
    ...
    attributes = np.array(
        [
            (b"Stale", b"Stale", f"{900 + i}".encode(), b"", 1.0, 9.0, b"Basic Mann n")
            for i in range(rows)
        ],
        dtype=attributes_dtype,
    )
    ineffective_dtype = np.dtype(
        [("Left Sta", "f4"), ("Right Sta", "f4"), ("Elevation", "f4"), ("Permanent", "?")]
    )
    with h5py.File(path, "a") as hdf:
        xs = hdf["Geometry/Cross Sections"]
        xs.create_dataset("Attributes", data=attributes)
        xs.create_dataset(
            "Manning's n Info",
            data=np.array([[3 * i, 3] for i in range(rows)], dtype=np.int32),
        )
        xs.create_dataset(
            "Manning's n Values",
            data=np.tile(
                np.array([[0.0, 0.2], [2.0, 0.2], [8.0, 0.2]], dtype=np.float32),
                (rows, 1),
            ),
        )
        xs.create_dataset(
            "Ineffective Info",
            data=np.array([[i, 1] for i in range(rows)], dtype=np.int32),
        )
        xs.create_dataset(
            "Ineffective Blocks",
            data=np.array([(7.0, 8.0, 555.0, True)] * rows, dtype=ineffective_dtype),
        )


def test_hybrid_stale_compound_table_reads_separated_datasets(tmp_path, monkeypatch):
    hdf_path = tmp_path / "Hybrid.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    _add_stale_compound_tables(hdf_path, rows=3)
    monkeypatch.setattr(HdfXsec, "_resolve_projection", lambda *_args, **_kwargs: None)

    with h5py.File(hdf_path, "r") as hdf:
        layout, rows, stale = HdfXsec._validated_cross_section_layout(
            hdf["Geometry/Cross Sections"]
        )
    assert (layout, rows) == ("hybrid", 2)
    assert stale == {"Attributes", "Manning's n Info", "Ineffective Info"}
    ...
    assert cross_sections["RS"].tolist() == ["200", "100"]
    assert cross_sections["Name"].tolist() == ["Upper", "Lower"]
    assert cross_sections["Left Bank"].tolist() == pytest.approx([2.0, 2.0])
    # Station Manning's n is preferred over the stale compound Manning's n pair.
    assert cross_sections.iloc[1]["mannings_n"]["Mann n"] == pytest.approx(
        [0.045, 0.035, 0.04, 0.05]
    )
    assert cross_sections["n_channel"].tolist() == pytest.approx([0.03, 0.035])
    # Blocked Ineffective is preferred over the stale compound Ineffective pair.
    assert cross_sections.iloc[0]["ineffective_blocks"] == [
        {"Left Sta": 0.0, "Right Sta": 1.5, "Elevation": 100.0, "Permanent": False}
    ]
    assert cross_sections.iloc[1]["ineffective_blocks"] == []
...
def test_hybrid_with_disagreeing_separated_anchors_fails_closed(tmp_path):
    hdf_path = tmp_path / "Hybrid.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    _add_stale_compound_tables(hdf_path, rows=3)
    with h5py.File(hdf_path, "a") as hdf:
        ...
        xs.create_dataset(
            "Station Elevation Info", data=np.array([[0, 3]], dtype=np.int32)
        )
...
def test_separated_only_legacy_layout_is_unchanged(tmp_path):
    hdf_path = tmp_path / "Legacy.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)

    with h5py.File(hdf_path, "r") as hdf:
        layout = HdfXsec._validated_cross_section_layout(hdf["Geometry/Cross Sections"])

    assert layout == ("legacy", 2, frozenset())
    assert len(HdfXsec.get_cross_sections(hdf_path)) == 2


def test_current_layout_reads_as_before_even_with_separated_extras(tmp_path, monkeypatch):
    hdf_path = tmp_path / "Modern.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    _convert_fixture_to_modern_schema(hdf_path)
    monkeypatch.setattr(HdfXsec, "_resolve_projection", lambda *_args, **_kwargs: None)
    baseline = HdfXsec.get_cross_sections(hdf_path)

    with h5py.File(hdf_path, "r") as hdf:
        layout = HdfXsec._validated_cross_section_layout(hdf["Geometry/Cross Sections"])
    assert layout == ("compound", 2, frozenset())

    # A compound table that agrees with the separated anchors is not a hybrid:
    # the compound Attributes and Manning's n pair stay authoritative.
    with h5py.File(hdf_path, "a") as hdf:
        xs = hdf["Geometry/Cross Sections"]
        xs.create_dataset("River Stations", data=np.array([b"X", b"Y"], dtype="S8"))
        xs.create_dataset("Node Names", data=np.array([b"X", b"Y"], dtype="S16"))
        xs.create_dataset(
            "Station Manning's n Info", data=np.array([[0, 1], [1, 1]], dtype=np.int32)
        )
        xs.create_dataset(
            "Station Manning's n Values",
            data=np.array([[0.0, 0.9], [0.0, 0.9]], dtype=np.float32),
        )
    with h5py.File(hdf_path, "r") as hdf:
        layout = HdfXsec._validated_cross_section_layout(hdf["Geometry/Cross Sections"])
    assert layout[0] == "compound"
    extras = HdfXsec.get_cross_sections(hdf_path)

    assert extras["RS"].tolist() == baseline["RS"].tolist() == ["200", "100"]
    assert extras["Len Channel"].tolist() == pytest.approx([110.0, 0.0])
    assert [row["Mann n"] for row in extras["mannings_n"]] == [
        row["Mann n"] for row in baseline["mannings_n"]
    ]
    assert extras.iloc[0]["ineffective_blocks"][0]["Permanent"] is True


def test_current_layout_row_mismatch_is_not_rescued(tmp_path):
    hdf_path = tmp_path / "Modern.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    _convert_fixture_to_modern_schema(hdf_path)
    with h5py.File(hdf_path, "a") as hdf:
        xs = hdf["Geometry/Cross Sections"]
        del xs["Manning's n Info"]
        xs.create_dataset("Manning's n Info", data=np.array([[0, 3]], dtype=np.int32))

    with pytest.raises(ValueError, match="row count mismatch"):
        HdfXsec.get_cross_sections(hdf_path)


def test_corrupt_compound_table_raises(tmp_path):
    hdf_path = tmp_path / "Corrupt.g01.hdf"
    _write_legacy_geometry_hdf(hdf_path)
    _convert_fixture_to_modern_schema(hdf_path)
    with h5py.File(hdf_path, "a") as hdf:
        xs = hdf["Geometry/Cross Sections"]
        del xs["Attributes"]
        xs.create_dataset("Attributes", data=np.zeros((2, 3), dtype=np.float32))
...
def test_unreadable_file_raises(tmp_path):
    hdf_path = tmp_path / "Truncated.g01.hdf"
...
def test_only_a_genuinely_absent_table_reads_empty(tmp_path):
    no_group = tmp_path / "TwoD.g01.hdf"
    with h5py.File(no_group, "w") as hdf:
        hdf.create_group("Geometry/2D Flow Areas")
    assert HdfXsec.get_cross_sections(no_group).empty

    zero_rows = tmp_path / "Empty.g01.hdf"
    with h5py.File(zero_rows, "w") as hdf:
        hdf.create_group("Geometry/Cross Sections").create_dataset(
            "Attributes", data=np.zeros((0,), dtype=np.dtype([("RS", "S8")]))
        )
    assert HdfXsec.get_cross_sections(zero_rows).empty
